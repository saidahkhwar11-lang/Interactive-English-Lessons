...
function openShareFolder(i){
  ...
  let rows=files.length?files.map((f,j)=>`<div class="clean-upload"><div class="file-icon">📄</div><div class="file-info"><b>${esc(f.name||'File')}</b><small>${f.date?esc(new Date(f.date).toLocaleDateString()):'Saved file'}</small></div><div class="file-actions">${f.data?`<button class="btn open-btn" onclick="openShareFile(${i},${j})">Open</button>`:`<button class="btn open-btn" disabled>Open</button>`}<button class="btn delete-btn" onclick="deleteShareFile(${i},${j})">Delete</button></div></div>`).join(''):'<div class="empty-state">This folder has no files yet.</div>';

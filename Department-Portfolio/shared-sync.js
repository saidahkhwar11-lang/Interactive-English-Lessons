...
async function startPortfolioAuth(){
 ...
 try{const {data,error}=await client.auth.getSession();if(error)throw error;if(data&&data.session)showSignedIn(data.session.user);else showLogin()}catch(e){console.warn("Initial session check delayed/failed",e);showLogin()}
 client.auth.onAuthStateChange((event,session)=>{
   ...
   if(event==="SIGNED_OUT"){window.PORTFOLIO_USER=null;showLogin()}
 });
}
...
async function signIn(){
 ...
 try{
   ...
   if(data&&data.session)showSignedIn(data.user||data.session.user);else msg.textContent="Sign-in was not completed. Please try again.";
...
function showSignedIn(user){const overlay=document.getElementById("portfolioLogin");if(overlay)overlay.remove();window.PORTFOLIO_USER=user;console.log("English Department Portfolio signed in:",user&&user.email)}
})();
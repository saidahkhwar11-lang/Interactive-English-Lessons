...
const dueSoon=x=>{if(!x.due||x.done)return false;const d=(new Date(x.due+'T00:00:00')-new Date(today()+'T00:00:00'))/86400000;return d>=0&&d<=2};
const overdue=x=>!x.done&&x.due&&x.due<today();
function summary(){const a=read(),active=a.filter(x=>!x.done);return {active:active.length,overdue:active.filter(overdue).length,soon:active.filter(dueSoon).length}}
window.renderTodoSummary=()=>{const e=document.getElementById('todoSummary');if(!e)return;const s=summary();e.innerHTML=`<b>${s.active} Active</b><span>🔴 ${s.overdue} Overdue</span><span>🟡 ${s.soon} Due Soon</span>`};
...
function taskRow(x){const status=x.done?'Completed':overdue(x)?'Overdue':dueSoon(x)?'Due Soon':'Upcoming';return `<div class="item"><div style="display:flex;justify-content:space-between;gap:12px;align-items:start"><div><b>${esc(x.title)}</b><small>${esc(x.assign)} · ${x.due?esc(x.due):'No due date'} · ${esc(x.priority)} · ${status}</small></div><div class="actions" style="margin:0"><button class="btn ${x.done?'':'primary'}" onclick="toggleTodo(${x.id})">${x.done?'Reopen':'Complete'}</button><button class="btn danger" onclick="deleteTodo(${x.id})">Delete</button></div></div></div>`}
...
window.installDepartmentTodo=()=>{if(document.getElementById('departmentTodoCard'))return;const grid=document.querySelector('.systemgrid');if(!grid)return;const card=document.createElement('article');card.id='departmentTodoCard';card.className='card support-card todo-card';card.innerHTML=`<div class="card-intro"><h3>📋 Department To-Do</h3><div id="todoSummary" class="todo-summary"></div></div><div class="actions"><button class="btn primary" onclick="openTodoAdd()">＋ Add Task</button><button class="btn" onclick="openTodoList()">View All</button></div>`;grid.appendChild(card);renderTodoSummary()};
